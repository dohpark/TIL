...
- zombie
  ...
  - process가 terminate되면 os에 의해 리소스들이 비할당됨. 근데, its entry int the process table must remain there until the parent calls wait(). 왜냐하면 the process table contains the process's exit status
